...
static CODE_FFI_EXPORT_FAILED: LazyLock<Code> = LazyLock::new(|| Code::must_new("arrow_ffi_export_failed"));
static CODE_FFI_IMPORT_FAILED: LazyLock<Code> = LazyLock::new(|| Code::must_new("arrow_ffi_import_failed"));
static CODE_BATCH_REBUILD_FAILED: LazyLock<Code> = LazyLock::new(|| Code::must_new("arrow_batch_rebuild_failed"));
static CODE_FFI_SCHEMA_EXPORT_FAILED: LazyLock<Code> =
    LazyLock::new(|| Code::must_new("arrow_ffi_schema_export_failed"));
static CODE_FFI_SCHEMA_IMPORT_FAILED: LazyLock<Code> =
    LazyLock::new(|| Code::must_new("arrow_ffi_schema_import_failed"));
static CODE_FFI_SCHEMA_NOT_STRUCT: LazyLock<Code> = LazyLock::new(|| Code::must_new("arrow_ffi_schema_not_struct"));
...
pub(crate) fn convert_batch(batch: &duckdb::arrow::record_batch::RecordBatch) -> Result<RecordBatch, Error> {
    ...
    for column in batch.columns() {
        ...
        let (ffi_array, ffi_schema) = to_ffi(&data).map_err(|e| {
            Error::wrap_internal(e, CODE_FFI_EXPORT_FAILED.clone(), "failed to export duckdb arrow array via FFI")
        })?;
        ...
        let imported = unsafe { arrow_array::ffi::from_ffi(ffi_array, &ffi_schema) }.map_err(|e| {
            Error::wrap_internal(e, CODE_FFI_IMPORT_FAILED.clone(), "failed to import arrow array via FFI")
        })?;
    ...
    RecordBatch::try_new(schema, columns)
        .map_err(|e| Error::wrap_internal(e, CODE_BATCH_REBUILD_FAILED.clone(), "failed to rebuild record batch"))
}
...
pub(crate) fn convert_schema_ref(schema: &duckdb::arrow::datatypes::Schema) -> Result<Arc<Schema>, Error> {
    ...
    let ffi_schema = duckdb::arrow::ffi::FFI_ArrowSchema::try_from(schema)
        .map_err(|e| Error::wrap_internal(e, CODE_FFI_SCHEMA_EXPORT_FAILED.clone(), "failed to export arrow schema via FFI"))?;
    ...
    let imported: arrow_schema::DataType = (&ffi_schema).try_into().map_err(|e: arrow_schema::ArrowError| {
        Error::wrap_internal(e, CODE_FFI_SCHEMA_IMPORT_FAILED.clone(), "failed to import arrow schema via FFI")
    })?;
    match imported {
        arrow_schema::DataType::Struct(fields) => Ok(Arc::new(Schema::new(fields))),
        other => Err(Error::new_internal(
            CODE_FFI_SCHEMA_NOT_STRUCT.clone(),
